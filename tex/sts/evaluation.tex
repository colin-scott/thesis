...
\begin{tabular}{|l||l|l|r|r|r|l|}
  ...
  \multirow{7}{*}{\rotatebox[origin=c]{90}{\textbf{Newly Found}}}
...
\hline
\hline
\multirow{3}{*}{\rotatebox[origin=c]{90}{\textbf{Known}}}
...
\hline
\hline
\multirow{7}{*}{\rotatebox[origin=c]{90}{\textbf{Synthetic}}} & Null pointer on
...
quantitatively show how well \projectname~reduces execution traces,
...
\noindent{\textbf{Pyretic Loop.}} We discovered a loop when fuzzing Pyretic's hub
module, whose purpose is to flood packets along a minimum spanning tree. After
reducing the execution (runtime in Figure~\ref{fig:pyretic_loop}), we found that the triggering event
...
%\begin{figure}[t]
%    %\hspace{-10pt}
%    \includegraphics{../graphs/runtime/pyretic_loop.pdf}
...
\noindent{\textbf{POX List Removal.}}
%The first
...
\begin{figure}[t]
    %\hspace{-10pt}
    \includegraphics{../graphs/runtime/list_remove_error.pdf}
    \caption{\label{fig:list_runtime} Reducing the POX list remove trace.}
\end{figure}
...
\noindent{\textbf{POX Premature PacketIn.}} We discovered this bug accidentally
...
without being able to repeat the experiment with a reduced trace. Making heavy
...
\end{subfigure}
\caption{Execution reduction results.}
\end{figure*}

\noindent{\textbf{POX In-flight Blackhole.}} We discovered another bug
...
We were able to reliably replay it by employing multiplexed sockets,
overriding {\texttt{gettimeofday()}},
...
\begin{figure}[t]
    %\hspace{-10pt}
    \includegraphics{../diagrams/state_machines/mesh_topo.pdf}
...
\noindent{\textbf{POX Migration Blackhole.}} We noticed after examining POX's code
...
reconstruct from the reduced execution trace how NOX should have installed routes, then compare
...
% -----------------------------------
\noindent{\textbf{ONOS distributed database locking.}} When testing ONOS, a distributed open-source controller, we noticed
that ONOS controllers would occasionally reject switch attempts to connect upon
initialization. The initial trace was already reduced, as the initial input
...
\noindent{\textbf{Floodlight loop.}} Next we
...
%\begin{figure}[t]
%    %\hspace{-10pt}
%    \includegraphics{../graphs/runtime/floodlight_loop.pdf}
...
a significantly reduced execution, we were able to diagnose the root cause---a
...
\projectname's ability to reduce traces on a range of bug types,
both known and synthetically injected by us.\\[0.5ex]
%
\noindent{\textbf{Floodlight failover bug.}} We were able to reproduce a
...
\noindent{\textbf{ONOS master election bug.}} We reproduced another bug,
...
\projectname{} was able to reduce the trace to 3 elements.\\[0.5ex]
\noindent{\textbf{POX load balancer error checking.}} We are aware that POX
...
session state to be lost. We were able to reduce the execution for this
...
%
\noindent{\textbf{Delicate timer interleaving.}}
...
%
\noindent{\textbf{Algorithm misimplementation.}}
...
%
\noindent{\textbf{Overlapping flow entries.}}
...
the switch. We were able to reduce 27 inputs to the two traffic injection
inputs necessary to trigger the routing entry overlap.\\[0.5ex]
%
\noindent{\textbf{Null pointer on rarely used codepath.}}
On a rarely used-code path, we injected a null pointer exception,
and were able to successfully reduce a fuzz trace of 62 events to the
expected conditions that triggered that code path: control channel congestion followed
by decongestion.\\[0.5ex]
%
\noindent{\textbf{Multithreaded race condition.}}
...
subsequence, we were able to reduce a 1596 input in 10 hours 15 minutes to a
...
inputs events.\\[0.5ex]
%
\noindent{\textbf{Memory leak.}}
We created a case that would take \projectname~very long to reduce: a
...
%
\noindent{\textbf{Memory corruption.}}
...
approach on the reduction of a synthetic non-deterministic loop created by
...
\hline
{\textbf{Max replays per subsequence}} & {\textbf{Size of final MCS}} & {\textbf{Total hours}}\\\hline\hline
1 & 65 & 6.10 \\\hline
2 & 20 & 6.37 \\\hline
3 & 15 & 7.78 \\\hline
4 & 12 & 9.59 \\\hline
...
\eat{
\begin{figure}[t]
    \includegraphics[trim=0 0 0 0]{../graphs/non-determinism/replays.pdf}
...
to redirect {\texttt{gettimeofday()}}, interpose on logging
...
\begin{figure}[t]
    \centering
    %\hspace{-10pt}
    \includegraphics{../graphs/scalability/scaling.pdf}
...
Throughout our experimentation we found that \projectname~leaves open several
parameters that need to be set properly in order to effectively find and troubleshoot bugs.

\noindent{\textbf{Setting fuzzing parameters.}} \projectname's fuzzer allows the
...
%
\noindent{\textbf{Differentiating persistent and transient violations.}} In
...
\noindent{\textbf{Setting \textepsilon.}} Our algorithm leaves an open question as to what value
...
\noindent{\textbf{Frenetic.}} Our first example is Frenetic~\cite{frenetic}, a control platform
...
\noindent{\textbf{POX}} Our second example is POX~\cite{pox}. POX is modeled after
...
Because POX is an early stage project, we chose to
fabricate a bug in POX's distribution failover logic, and independently
validate that \projectname~was able to identify, prioritize, and find a
reduced execution
for the fabricated correctness violation.
...
\noindent{\textbf{Floodlight: Distributed controller failover race condition}}
...
\noindent{\textbf{Record and Replay Overhead.}} In contrast to general record-and-replay
...
\noindent{\textbf{Correspondence Checking Runtime.}} Computing the propagation
...
\begin{figure}[t]
    %\hspace{-10pt}
    \includegraphics{../graphs/hsa_overhead_graph/graph.pdf}
...
\noindent{\textbf{Simulator Scalability.}} Our design models the entire network
...
\begin{figure}[t]
    %\hspace{-10pt}
    \includegraphics{../graphs/scalability_graph/scale.pdf}

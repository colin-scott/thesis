...
\eat{
\noindent{\textbf{Aren't SDN controllers relatively simple and bug free?}}
...
\noindent{\textbf{Aren't the bugs described here trivial in nature?}} Yes, the bugs we found were trivial, but that
...
\noindent{\textbf{Are simulated failures really indistinguishable from actual
...
\noindent{\textbf{How much effort do MCSes really save?}} Based on conversations
...
\noindent{\textbf{Will this approach work on all controllers?}} We make limited
...
{\texttt{gettimeofday()}}) can increase replay accuracy further.
In general, we expect \projectname~to support controllers conforming to
OpenFlow~1.0. % out of the box.

\noindent{\textbf{Why do you focus on SDN?}} SDN represents both an
...
In chapter~\ref{sec:demi}, we show the extent to which the ideas embodied
in~\projectname~are applicable to other types of distributed systems.
...
\eat{
\noindent{\textbf{Dataflow analysis.}} Another avenue for future work would be to apply
dataflow
...
\eat{
\noindent{\textbf{Optimizing delta debugging.}} Delta debugging is agnostic to how
...
\noindent{\textbf{Enabling analysis of production executions.}} \projectname~does
not currently support reduction of production (as opposed
to QA) executions. Production systems would need to include Lamport
clocks on each message~\cite{Lamport:1978:TCO:359545.359563} or have
sufficiently accurate clock synchronization to obtain a partial
(happens-before) ordering of the message deliveries in the execution. Inputs would also
...
\eat{ % LONG VERSION
\projectname~does not currently support reduction of production (as opposed
to QA) logs.
Here we present a sketch of how \projectname~might support production logs as input.

While we take as input a single, totally-ordered trace of the events in the
...
\noindent{\textbf{How big is the log? How long does this take to run?}}

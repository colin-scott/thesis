...
While program flow reduction is able to reduce inputs rather than thread context switches,
...
not attempt to reduce them. Crucially though, we need to ensure that the ordering
...
(reduction opportunities) to delta debugging as often as possible.
...
\noindent{\textbf{Functional Equivalence.}} Internal events may differ syntactically (\eg~sequence numbers
of control packets may all differ) when replaying a subsequence of the
...
may be absent when replaying a subsequence of that execution.
...
events before we replay each subsequence. Our algorithm (called {\textsc{Peek()}}) for inferring the
...
\noindent{\textbf{Recap.}} We combine the above heuristics to replay each

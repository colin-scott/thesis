...
% TODO(cs): cite "Leveraging SDN layers" paper?
...
\noindent{\textbf{Input Minimization for Sequential Programs.}} Minimization
...
\noindent{\textbf{Minimization without Interposition.}} Several
...
\noindent{\textbf{Thread Schedule Minimization.}} Other techniques seek to minimize thread
...
\noindent{\textbf{Thread Schedule Minimization Augmented with Program Analysis.}} By analyzing the program's control- and
...
techniques in the general area of troubleshooting.
...
distributed
systems~\cite{lin2009modist,leesatapornwongsa2014samc,simsa2010dbug}, which
interpose on timers and message send and receive events.

%MoDist does not make assumptions about the language of the
%system under test. As we discussed in \S\ref{subsec:generality}, interposing
%at the message layer has several advantages over interposing at a lower layer

% As we discussed in \S\ref{}...
...
% TODO(cs): say something about STS' interposition? Similar to Mininet, but we
% focus not on performance, and instead on fine grained control over timings..

\noindent{\textbf{Bug Reproduction.}} Record and replay
techniques seek to reproduce a given concurrency bug~\cite{zamfir2011debug,Zamfir:2010:EST:1755913.1755946,altekar2009odr,park2009pres,Yuan:2010:SED:1736020.1736038}. One a bug has been
reproduced, users can step through the (deterministic) execution and interactively examine the
state of the system. Within networking, OFRewind~\cite{ofrewind} provides
...
\noindent{\textbf{Probabilistic Diagnosis.}} To avoid the runtime overhead of deterministic replay, other techniques
...
% TODO(cs): update this to include material from the blog post.
\noindent{\textbf{Instrumentation.}} Unstructured
...
% TODO(cs): update this to include material from the blog post.
\noindent{\textbf{Bug Detection.}} With instrumentation available, it becomes possible

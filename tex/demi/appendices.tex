...
\noindent{\bf Soundness:} any executed schedule should be valid, i.e. possible
to execute on an uninstrumented version of the program starting from the
initial configuration.

\noindent{\bf Efficiency:} the happens-before partial order for every executed schedule
should never be a prefix of any other partial orders that have been
previously explored.

\noindent{\bf Completeness:} when the state space is acyclic, the strategy is guaranteed to
...
\noindent{\bf raft-45: Candidates accept duplicate votes from the same
...
\noindent{\bf raft-46: Processes neglect to ignore certain
...
\noindent{\bf raft-56: Nodes forget who they voted for.} akka-raft is written as a finite state machine.
...
\noindent{\bf raft-58a: Pending client commands delivered
...
\noindent{\bf raft-58b: Ambiguous log indexing.} In one of our fuzz tests,
...
\noindent{\bf raft-42: Quorum computed incorrectly.} We also found a fuzz test that
...
\noindent{\bf raft-66: Followers unnecessarily overwrite log entries.} The last issue we found is only possible to
...
% ------------------- spark-2294 -------------------
\noindent{\bf spark-2294: Locality inversion.} In Spark, an `executor' is
...
% ------------------- spark-3150 -------------------
\noindent{\bf spark-3150: Simultaneous failure causes infinite restart loop.} Spark's
...
\noindent{\bf spark-9256: Delayed message causes master crash.} We found the

...
\noindent{\bf Fuzz testing with \sys.} We begin by using \sys~to generate
...
\noindent{\bf Mitigating non-determinism.} Processes may behave non-deterministically.
...
\noindent{\bf akka-raft instrumentation.} Within akka-raft, actors use a
...
\noindent{\bf Spark instrumentation.} Within Spark, the task scheduler chooses the first value from a hashmap in order
...
\noindent{\bf Akka changes.} In a few places within the Akka framework, Akka assigns IDs using an
...
\noindent{\bf Stop-gap: replaying multiple times.} In cases where it is
...
\noindent{\bf Blocking operations.} Akka deviates from the computational model we defined
...
\noindent{\bf Safety vs.\ liveness.} We
are primarily focused on safety violations, not liveness or performance bugs.

\noindent{\bf Non-Atomic External Events.} \sys~currently waits for external
...
\noindent{\bf Limited scale.} \sys~is currently tied to a
...
\noindent{\bf Shared memory \& disk.} In some systems processes communicate
by writing to
...
\noindent{\bf Generality of \sys.} We targeted the Akka actor framework
...
\noindent{\bf Generality of scheduling strategies.} At their core, distributed
